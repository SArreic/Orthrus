...
# 在source scripts/initialize-deployment.sh之后添加：
enable_rl=false
rl_config=""
...
if [ "$depl_type" = "local" ]; then
    source scripts/deploy-local.sh --rl $enable_rl --rl-config "$rl_config"
elif [ "$depl_type" = "cloud" ]; then
    source scripts/deploy-cloud.sh --rl $enable_rl --rl-config "$rl_config"
fi
